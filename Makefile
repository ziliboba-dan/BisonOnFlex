...
test:
	...
	clang++  -std=c++11 generate/parser.tab.c generate/lex.yy.c test/octalhex.cpp -o bin/octalhex -lfl
	clang++  -std=c++11 generate/parser.tab.c generate/lex.yy.c test/error.cpp -o bin/error -lfl
#clang++  -std=c++11 generate/parser.tab.c generate/lex.yy.c test/test.cpp -o bin/test_if -lfl
	...
	./bin/octalhex < test/octalhex.go
	./bin/error < test/error.go
	rm bin/test*

...
test:
	...
	clang++  -std=c++11 generate/parser.tab.c generate/lex.yy.c test/test_error.cpp -o bin/test_error -lfl
	./bin/test_if < test/test_if.go
	./bin/test_compute < test/test_compute.go
	./bin/test_loop < test/test_loop.go
	./bin/test_error < test/test_error.go
	rm bin/test*

.PHONY: tidy
tidy:
	clang-tidy **/*.cpp -checks=-*,clang-analyzer-*,-clang-analyzer-cplusplus* -- -DDEBUG -I./include
